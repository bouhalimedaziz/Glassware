  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("[v0] Products fetched:", products)

        const cats = await getCategories()
        console.log("[v0] Categories fetched:", cats)

        const featuredProducts = products.filter((p) => p.isFeatured).slice(0, 5)
        console.log("[v0] Featured products:", featuredProducts.length)

        setFeatured(featuredProducts.length > 0 ? featuredProducts : products.slice(0, 5))
        setCategories(cats)
      } catch (error) {
        console.error("[v0] Home fetch error:", error)
        setFeatured([])
        setCategories([])
      } finally {
        setLoading(false)
