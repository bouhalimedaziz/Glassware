...
export default function Shop() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("[v0] Fetching shop products")
        const allProducts = await getAllProducts()
        console.log("[v0] Shop products fetched:", allProducts)

        const cats = await getCategories()
        console.log("[v0] Shop categories:", cats)

        setProducts(allProducts)
        setCategories(["All", ...cats])
      } catch (error) {
        console.error("[v0] Shop fetch error:", error)
        setProducts([])
        setCategories(["All"])
      } finally {
        setLoading(false)
